...
(ii) HTTPS data exfiltration & Step 3 & Upload volume + down/up ratio \\
(ii) DNS data exfiltration & Step 5 & DNS query volume \\
(iii) C\&C activities using DNS & Step 5 (volume) + Step 6 (DNS timing) & Volume anomaly + timing regularity \\
...
amount of data outbound over HTTPS using two complementary sub-rules.

\begin{itemize}
\tightlist
\item
  \textbf{Sub-rule A (Volume):} flags any client uploading more than
  mean + 3σ (116.6 MB) in the test period. Measuring cumulative volume
  over the full observation window is a deliberate design choice: a
  low-and-slow exfiltrator stays invisible to rate-based rules, but its
  data accumulates and eventually crosses the total volume threshold.
\item
  \textbf{Sub-rule B (Down/up ratio):} flags any client whose HTTPS
  down/up ratio falls below mean − 3σ (threshold: 8.56). A normal
  internal client downloads roughly 9.24× what it uploads (browsing
  pattern). An exfiltrating client inverts this balance: it uploads
  heavily while downloading little, producing a dramatically low ratio
  regardless of absolute byte count. This catches devices whose
  exfiltration volume alone stays below the 116.6 MB threshold.
\end{itemize}

A third signal, the PCR (Producer-Consumer Ratio), was implemented and
...
across the threshold. PCR was removed in favour of the direct ratio
sub-rule above.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Sub-rule A: upload volume}
\NormalTok{vol\_flag   }\OperatorTok{=}\NormalTok{ per\_ip[}\StringTok{\textquotesingle{}total\_up\textquotesingle{}}\NormalTok{] }\OperatorTok{\textgreater{}}\NormalTok{ vol\_threshold   }\CommentTok{\# \textgreater{} 116.6 MB}
\CommentTok{\# Sub-rule B: down/up ratio}
\NormalTok{ratio\_flag }\OperatorTok{=}\NormalTok{ per\_ip[}\StringTok{\textquotesingle{}ratio\textquotesingle{}}\NormalTok{] }\OperatorTok{\textless{}}\NormalTok{ ratio\_low          }\CommentTok{\# \textless{} 8.56  (mean-3σ)}
\NormalTok{flagged    }\OperatorTok{=}\NormalTok{ per\_ip[vol\_flag }\OperatorTok{|}\NormalTok{ ratio\_flag]}
\end{Highlighting}
\end{Shaded}

Both thresholds are derived from the training distribution of internal
HTTPS clients. The ratio mean is 9.24 with std 0.23, computed across all
internal HTTPS traffic; the volume mean is 20.4 MB with std 32.1 MB.

\subsubsection{Results}

The rule flagged 10 internal IPs across the two sub-rules:

\begin{verbatim}
[ALERT] 192.168.101.187  upload=7586 MB (316σ); ratio=0.14 (40.2σ)
[ALERT] 192.168.101.14   upload=5352 MB (223σ); ratio=0.05 (40.5σ)
[ALERT] 192.168.101.208  upload=4402 MB (183σ); ratio=0.11 (40.3σ)
[ALERT] 192.168.101.26   upload=259 MB  (9σ);   ratio=1.97 (32.1σ)
[ALERT] 192.168.101.197  upload=138 MB  (4σ);   ratio=2.38 (30.3σ)
[ALERT] 192.168.101.207  upload=119 MB  (3σ)
[ALERT] 192.168.101.188  ratio=3.80 (24.0σ)
[ALERT] 192.168.101.117  ratio=8.23  (4.5σ)
[ALERT] 192.168.101.128  ratio=8.45  (3.5σ)
[ALERT] 192.168.101.78   ratio=8.51  (3.2σ)
\end{verbatim}

The first five IPs are flagged by both sub-rules simultaneously: their
ratios range from 0.05 to 2.38, meaning they download only 0.05--2.38×
what they upload — the clearest possible exfiltration signature,
independently confirmed by both volume and ratio at 30--40σ deviation.
The top three (.187, .14, .208) represent bulk data dumps at 4.4--7.6 GB,
183--316σ above baseline.

Four IPs are flagged exclusively by the ratio sub-rule. .188 is the most
significant at 24.0σ: it uploads heavily relative to its downloads while
keeping volume below the 116.6 MB threshold — exactly the profile of a
targeted low-and-slow exfiltrator. .117 (4.5σ), .128 (3.5σ) and .78
(3.2σ) show a progressively less extreme but still statistically clear
upload bias.

\medskip
\noindent\colorbox{gray!12}{\parbox{\dimexpr\linewidth-2\fboxsep\relax}{\textbf{Spec requirement covered:} (ii) HTTPS data exfiltration via upload volume (sub-rule A) and down/up ratio anomaly (sub-rule B).}}
...
\ControlFlowTok{if}\NormalTok{ new\_asns }\KeywordTok{and}\NormalTok{ flows }\OperatorTok{\textgreater{}=}\NormalTok{ baselines[}\StringTok{\textquotesingle{}new\_asn\_min\_flows\textquotesingle{}}\NormalTok{]:  }\CommentTok{\# p10 training = 5}
    \CommentTok{\# fire alert}
...
\ControlFlowTok{if}\NormalTok{ new\_dsts }\KeywordTok{and}\NormalTok{ flows }\OperatorTok{\textgreater{}=}\NormalTok{ baselines[}\StringTok{\textquotesingle{}new\_int\_min\_flows\textquotesingle{}}\NormalTok{]:  }\CommentTok{\# p10 training = 108}
    \CommentTok{\# fire alert}
\end{Highlighting}
\end{Shaded}
...
\medskip
\noindent\colorbox{gray!12}{\parbox{\dimexpr\linewidth-2\fboxsep\relax}{\textbf{Spec requirement covered:} (ii) DNS data exfiltration via query volume anomaly + (iii) C\&C activities using DNS via timing regularity (Step 6).}}
...
Running the full detection pipeline against the test dataset produced 33
unique anomalous IPs: 23 internal clients (192.168.101.x) and 10 external
...
The 9 HIGH confidence IPs are therefore treated as confirmed compromised
...
\begin{verbatim}
 Total Unique Anomalous IPs: 33
    - Internal (192.168.101.x): 23
  ...
  [HIGH  ] 192.168.101.72
             New Country Destinations | New Destination IPs / ASNs
             | External Destination Fan-out
  [HIGH  ] 192.168.101.117       HTTPS Data Exfiltration | BotNet Beaconing
  [HIGH  ] 192.168.101.125
             New Country Destinations | New Destination IPs / ASNs
             | External Destination Fan-out
  [HIGH  ] 192.168.101.188       HTTPS Data Exfiltration | BotNet Beaconing
  ...
  [MEDIUM] 192.168.101.78        HTTPS Data Exfiltration
  [MEDIUM] 192.168.101.93        New Source IPs
  [MEDIUM] 192.168.101.128       HTTPS Data Exfiltration
...
The 9 HIGH confidence devices are those flagged by two or more
...
192.168.101.188 & HTTPS Exfiltration (ratio) + BotNet & Down/up ratio
of 3.80 (24.0σ below baseline): uploading 3.8× what it downloads while
a confirmed HTTPS beacon fires at 1.03 s intervals — simultaneous
exfiltration and C2 \\
192.168.101.117 & HTTPS Exfiltration (ratio) + BotNet & Down/up ratio
of 8.23 (4.5σ below baseline) alongside HTTPS beaconing at 1.00 s
intervals (8.46× more regular than training): exfiltration confirmed
...
Grouping the 33 flagged devices by behaviour reveals nine distinct
...
single day, 183--316σ above baseline: bulk data dumps flagged by both
volume and ratio sub-rules simultaneously \\
Moderate HTTPS Exfiltration & .197, .26, .207 & 119--259 MB uploaded,
3--9σ above the 116.6 MB volume threshold \\
HTTPS Ratio Exfiltration & .188, .117, .128, .78 & Upload-heavy ratio
(3.2--24.0σ below baseline of 9.24): volume stays below the threshold
but the down/up imbalance reveals sustained exfiltration behaviour \\
...
One packet is sent per flagged IP. All 33 were successfully delivered to
172.100.0.12:514:
...
[*] Sending UEBA alerts to Wazuh (172.100.0.12:514)...
      ...
      ...
      → Alarm UEBA 192.168.101.188   (33 total)
\end{verbatim}
...
With the Wazuh stack running and all 33 syslog packets delivered,
...
33 hits, each carrying data.srcip with the anomalous device's address,
...
The UEBA module identified 33 anomalous devices (23 internal and 10
...
The nine HIGH-confidence detections, each corroborated by two or more
