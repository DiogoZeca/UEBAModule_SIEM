...
\hypertarget{ai-prompt}{%
\paragraph{AI Prompt}\label{ai-prompt}}

\hypertarget{all-the-context-above-was-given-to-the-ai-exactly-as-is-just-to-understand-the-goal-of-the-project.}{%
\subsection{\textgreater{} (All the context above was given to the AI
exactly as is just to understand the goal of the
project.)}\label{all-the-context-above-was-given-to-the-ai-exactly-as-is-just-to-understand-the-goal-of-the-project.}}
...
maintain nearly identical down/up ratios (\textasciitilde9.23 vs
\textasciitilde9.25) that means the clients download roughly 9.2 bytes
...
per-client upload mean of \textasciitilde45 MB is the direct sum of the
...
The gap between median (104s) and mean (807s), and the jump from p90
(191s) to p95 (3,475s), confirms a heavy-tailed distribution, consistent
with human web browsing: short active bursts separated by reading
pauses, with occasional long overnight idle gaps.

\hypertarget{ai-prompt-1}{%
\subsubsection{AI Prompt}\label{ai-prompt-1}}

\begin{quote}
Explore the data given in the dataset1 folder having in mind all the
context I gave you before, the script should read the training and then
output usefull information that we could use in order to achieve our
goal. The idea is to use a pythonScript, that you can have in base the
example (sampleScript.py) provided.
\end{quote}
...
This means scanning \textasciitilde900k rows five separate times and
...
Each threshold targets a different dimension of behaviour. - The HTTPS
upload threshold (116.6 MB) defines the maximum total data a normal
internal client sends over HTTPS in a day, anything above this suggests
bulk data exfiltration. The threshold is derived from combined HTTPS
traffic (internal .240 + external servers). The destination split
characterised above shows both groups have identical ratios
(\textasciitilde9.23), so the combined aggregate is a valid baseline. -
The DNS flow count threshold (1,399 flows) flags clients generating an
abnormal number of DNS queries, the primary signal for DNS-based C\&C
beaconing. - The BotNet interval p05 thresholds are split by protocol:
1,881.3 s for HTTPS traffic and 6,373.2 s for DNS traffic, representing
the 5th percentiles of per-client interval standard deviations computed
separately for each protocol. Any client below its protocol's floor is
more regular than 95\% of all legitimate training clients for that
protocol. Separating the protocols prevents a device with naturally
irregular HTTPS activity from masking a tight DNS beaconing pattern in a
combined calculation. - The external ratio window ({[}8.3817, 8.6226{]})
defines the expected down/up range for external clients. Any external
client outside this window is interacting with the corporate server in
an anomalous way. - The geo minimum intensity floor (10 flows) filters
CDN noise from the geo detection rule: any client reaching a country new
to the entire network with fewer than 10 flows is treated as a CDN edge
rotation rather than a genuine new destination. - The external fan-out
threshold (258 unique external IPs) flags any internal client contacting
an anomalous number of distinct external destinations in a single day.
Derived from mean (118.1) + 3σ (3 × 46.7), it catches sweep-style
behaviour where a host checks in with many scattered external endpoints
rather than concentrating traffic on the small set of servers typical of
legitimate browsing.
...
The alternatives were evaluated and rejected: - 2σ (97.7\% coverage)
would produce approximately 4 false positives per rule per day, creating
a noise floor that drowns real signals; - 4σ (99.9937\% coverage) would
set the bar so high that moderate exfiltration (a device uploading 200
MB when the mean is 45 MB) would go undetected.

\hypertarget{exception}{%
\subsection{Exception}\label{exception}}

The BotNet rule cannot use ``mean − 3σ'' because the inter-flow interval
standard deviation distribution in training is heavily right-skewed
(most clients have moderate regularity, but a few have very high
interval variability, pulling the mean up).

The training values are: mean = 8,350 s, std = 6,668 s. Applying ``mean
− 3σ'' gives 8,350 − 3 × 6,668 = −11,654 s; a negative threshold, which
is mathematically meaningless for a standard deviation. Instead, we use
the 5th percentile (p05 = 1,741.9 s) of the training distribution as the
floor. This represents the lower boundary of normal behaviour
empirically observed in training. Any client with an inter-flow interval
std below this value is more regular than 95\% of all normal clients,
making it a strong candidate for beaconing behaviour.
...
\hypertarget{ai-prompt-2}{%
\subsubsection{AI Prompt}\label{ai-prompt-2}}

\begin{quote}
Now we need to do a compute\_baselines() function to help us defining
the baselines. The idea is to compute the necessary statistics and
thresholds that will be used by the detection rules. You can use the
information you got from the data exploration phase and the list with
all rules and justification for detection, to decide which statistics
are relevant for each rule. The idea is to find the ``normal'' tresholds
for each rule based on the training data, so that when we apply the
rules to the test data, we can flag any deviations from these baselines
as anomalies.
\end{quote}
...
real anomalies with minimal false positives. \#\#\# Code Implementation
Code used:
...
Results The rule flagged 5 external IPs, which naturally split into two
...
The results revealed two groups: - The first group (.61, .64, .174) sit
below the lower bound. Their ratio is lower than normal, meaning they
upload proportionally more than a legitimate client would. This is
consistent with a compromised account being used to push data toward the
corporate server, or with reversed exfiltration where data is staged on
the server from outside. - The second group (.182, .210) sit above the
upper bound, downloading more than expected relative to what they
upload, consistent with bulk data retrieval or unusual large-object
access. These are two different threat models, but the same symmetric
rule catches both because both break the tight ratio invariant in
opposite directions.

The 188.83.72.61 at 6.7σ is the most extreme anomaly in the entire
external dataset.

A second detection signal was implemented and evaluated before
finalising this rule. Since compute\_baselines() already computes the
per-client inter-flow interval distribution for characterisation, the
p05 of per-client interval standard deviation (2,132.6 s) was added as a
second threshold alongside the ratio: any external client whose
inter-flow interval std fell below this floor (more regular than 95\% of
all training clients) would also be flagged, using OR logic.

Applied to the test data, this produced 25 flagged IPs instead of 5. The
additional 20 were triggered exclusively by the interval signal with
completely normal ratios. The problem is statistical: a p05 floor
applied to 196 test clients produces approximately 10 false positives by
chance alone, and the borderline interval-only hits (iv\_std ranging
1,839--2,130 s against a floor of 2,132.6 s) are too marginal to be
convincing detections.

The interval signal was removed and the floor retained as
characterisation output only.
...
near-identical per-group ratios (\textasciitilde9.23 in both)
...
rule through timing analysis. \#\#\# Code Implementation
...
(\textasciitilde9.23), confirming the combined aggregate is a valid
...
dropped in favour of the simpler volume threshold alone. \#\#\# Results
...
to this and produces noise, not signal. \#\#\# Code Implementation To
...
infrastructure, not just new geography. \#\#\# Results The global
...
contacts far more of them than any training client ever did. \#\#\# Code
Implementation
...
public-IP subset already used by the geo and ASN rules. \#\#\# Results
...
the exact 5.0s median interval across all flagged devices is the
...
discrimination power for this dataset. \#\#\# Results DNS-1 flagged 5
...
Every flagged device shows an exact 5.0s median interval between DNS
...
The first approach was to apply the standard mean − 3σ threshold,
flagging any client below this floor. This immediately broke down: the
inter-flow interval std distribution in training is heavily
right-skewed, with a mean of 8,350 s and a std of 6,668 s. Applying mean
− 3σ gives: \texttt{8,350\ −\ 3\ ×\ 6,668\ =\ −11,654\ s} A negative
standard deviation is mathematically meaningless. No threshold can be
set this way on a right-skewed distribution. \#\#\# Code Implementation
The solution was to use the 5th percentile (p05) of each protocol's
training distribution as the threshold floor. This represents the
empirical lower boundary of normal behaviour per protocol: any client in
the test period with an interval std below the HTTPS floor (1,881.3 s)
is more regular than 95\% of all legitimate HTTPS sessions in training;
similarly for DNS (6,373.2 s).
...
flagged. A device with train\_std = 2,000 s and test\_std = 1,800 s has
barely changed; a device with train\_std = 12,000 s and test\_std =
1,800 s has genuinely become more regular --- only the latter is
beaconing.
...
The 6 devices split cleanly by protocol: - The DNS beacons (.23, .41,
.201) fire at an exact 5.0 s median interval through the internal DNS
resolvers, consistent with Step 5's findings. The same malware heartbeat
confirmed by both volume and timing analysis. - The HTTPS beacons (.117,
.157, .188) fire at intervals clustering tightly between 100 and 103
seconds --- the fingerprint of a single malware family sharing the same
hardcoded C2 timer.
...
change: its training HTTPS interval std was \textasciitilde8,740 s
(irregular browsing); in the test period it dropped to 1,035 s (8.46×
...
combined-channel analysis because mixing their DNS 5 s intervals with
HTTPS \textasciitilde100 s intervals produced an artificially low
...
The 192.168.101.148, flagged by DNS-1 with 1,661 flows at a 5.0 s median
interval, is not flagged by this rule. Although its DNS query count
exceeds the volume threshold, its DNS interval std is above the 6,373 s
...
Running the full detection pipeline against the test dataset produced 24
unique anomalous IPs: 19 internal clients (192.168.101.x) and 5 external
...
The two-level scheme is not arbitrary: - when a single rule flags a
device, there is always a small residual probability of a false
positive; - when two completely independent detection methods both flag
the same IP on the same day, the probability of a coincidental double
false positive is the product of the individual rates, dropping to
approximately 0.0001\% per device.

The 7 HIGH confidence IPs are therefore treated as confirmed compromised
devices. The 17 MEDIUM confidence IPs are genuine anomalies that warrant
...
BotNet HTTPS interval std & 1844 s (.160, regularisation=1.17×) & 1881 s
& 37 s above floor \\
...
37 s above the 1,881 s floor, but its regularisation ratio is only
...
interval std of 6,326 s (47 s above the floor) and a DNS flow count
...
boundary is not cutting too close to normal behaviour.
...
5.0 s interval --- extreme C2 beaconing via internal DNS relay,
confirmed by both volume and timing \\
192.168.101.23 & DNS Volume + BotNet & 8,651 DNS queries at 5.0 s ---
same C2 pattern, smaller scale, same malware family \\
192.168.101.201 & DNS Volume + BotNet & 2,941 DNS queries at exact 5.0 s
...
contacted (3.3σ above mean); 1,418 DNS queries at an exact 5.0 s beacon
...
Grouping the 24 flagged devices by behaviour reveals eight distinct
...
DNS C\&C Beaconing & .41, .23, .201, .148 & Exact 5.0 s query interval
...
between 100--103 s across three independent devices --- single malware
family, shared hardcoded timer \\
Exfiltration + C2 Beacon & .207 & HTTPS exfiltration (119 MB, 3σ)
running alongside a DNS C2 beacon (1,418 queries at 5.0 s intervals) and
...
ratios (\textasciitilde1:1) inconsistent with web browsing ---
peer-to-peer command relay or lateral file transfer between compromised
hosts \\
...
manager by sending a UDP syslog message in the format ``Alarm UEBA ''
...
One packet is sent per flagged IP. All 24 were successfully delivered to
172.100.0.12:514:
...
With the Wazuh stack running and all 24 syslog packets delivered,
navigating to Security Events and applying the DQL filter rule.id:
100201 shows one event per flagged IP: 24 hits, each carrying data.srcip
